<template>
  <div>
    <h2 id="page-heading" data-cy="CompteHeading">
      <span v-text="$t('pathogeneApp.compte.home.title')" id="compte-heading">Comptes</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('pathogeneApp.compte.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CompteCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('pathogeneApp.compte.home.createLabel')"> Create a new Compte </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && comptes && comptes.length === 0">
      <span v-text="$t('pathogeneApp.compte.home.notFound')">No comptes found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('user.id')">
              <span v-text="$t('pathogeneApp.compte.user')">User</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="pathogeneApp.compte.delete.question" data-cy="compteDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-compte-heading" v-text="$t('pathogeneApp.compte.delete.question', { id: removeId })">
    ...
    <div v-show="comptes && comptes.length > 0">
