...
public class PatientResource {
     ...
     */
    @GetMapping("/patients")
    public ResponseEntity<List<Patient>> getAllPatients(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        ...
        if (accountResource.getAccount().getAuthorities().contains(AuthoritiesConstants.SECRETAIRE)) {
            // Récupérez les patients du référentiel
            Slice<Patient> slice = patientRepository.findAllWithEagerRelationships(pageable);

            // Appliquez le filtrage
            List<Patient> filteredPatients = slice
                .getContent()
                .stream()
                .filter(patient ->
                    patient.getSecretaire() != null &&
                    patient.getSecretaire().getId().equals(accountResource.getAccount().getSecretaire().getId())
                )
                .collect(Collectors.toList());

            // Créez une nouvelle page avec les résultats filtrés
            patients = new PageImpl<>(filteredPatients, pageable, ((Page<Patient>) slice).getTotalElements());
        } else {
            patients = patientRepository.findAllWithEagerRelationships(pageable);
        }
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), patients);
